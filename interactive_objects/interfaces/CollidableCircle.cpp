 ...
 * @return
 */
std::vector<CollidableObject *> CollidableCircle::collision_detection_objects(std::vector<CollidableObject *> collidableObjects, cv::Point2d* pos) {
    std::vector<CollidableObject *> collided_with;
    ...
    for (CollidableObject *object : collidableObjects) {
        ...
        switch (object->getObjectType()) {
            case CollidableObjectType::circle:
                if (collision_detection_circle_circle(this, static_cast<CollidableCircle *>(object), pos)) {
                    std::cout << "Collision with circle detected" << std::endl;
                    collided_with.push_back(object);
                }
        }
    }
